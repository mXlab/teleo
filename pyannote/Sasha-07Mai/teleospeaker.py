...
from pyannote_embeddings_familiarity import known_data, get_familiarity_scores

# Set the path to the Hugging Face authentication token
HUGGING_FACE_AUTH_TOKEN = ""

# Import or define the trust function if not already available
def trust(label):
    # Placeholder trust function, replace with actual logic
    return 1.0
...
class SpeakerRealTimeProcessing:
    ...
    def process_audio(self, data):
        ...
        # Access familiarity_scores from pyannote_embeddings_familiarity
        familiarity_scores = get_familiarity_scores()
        print(f"Familiarity Scores: {familiarity_scores}")

        # Use known_data as embeddings
        embeddings = known_data
        labels = np.arange(len(embeddings))  # Generate placeholder labels for known_data

        w_trust = compute_voice(embedding, embeddings, labels)
        print(f"Computed w_trust: {w_trust}")
